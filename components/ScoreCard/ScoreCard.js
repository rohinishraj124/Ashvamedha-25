    ...
    if (window.confirm("Delete the livescore??")) {
      try {
        const res = await axios.delete(`${server}/sport/deletelivescore`, {
          data: { matchId: props.info._id }
        });
        console.log(res);
        if (res.data.statusCode === 200) {
          alert(`Live Score Ended for MatchName : "${props.info.matchName}", Update the points table`);
          router.push("/updatepointstable"); // Use router.push for navigation
        }
      } catch (error) {
        console.error("Failed to delete the score:", error);
        alert("Error: Could not delete the score. Please try again.");
      }
    }
  };
    ...
    <div className="border-2 border-white w-11/12 sm:w-3/5 mb-12 text-white rounded-2xl pb-4 bg-[#090909] flex flex-col-reverse justify-start items-center">
      ...
      {(loginCtx.sport === props.info.sportName) && (
        <div className="mt-4">
          ...
          <button 
            onClick={() => router.push(`/updatelivescore/${props.info._id}`)} 
      ...
      <div className="h-full w-full sm:w-4/5 flex justify-around items-center mt-5 px-2">
        ...
        <div className="flex flex-col justify-evenly items-center text-center text-lg sm:text-2xl w-1/3">
          <div className="relative border-2 border-white rounded-full w-[80px] h-[80px] sm:w-[150px] sm:h-[150px] flex justify-center items-center overflow-hidden">
